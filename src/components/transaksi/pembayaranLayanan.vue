    ...
    <v-container>
        ...
        <v-dialog v-model="dialogPenitipan" persistent max-width="300px">
            <v-card>
                ...
                <v-text-field :hint="'Jumlah Sebelumnya : '+formEdit.jumlah+' hari'" v-model="jlhHari" class="ml-4 mr-4" :rules="rules.jumlah" placeholder="Jumlah Hari" outlined type="number"></v-text-field>
                <v-card-actions>
                    <v-spacer></v-spacer>
...
export default {
    ...
    methods:{
        getJenis(){
            var uri = this.$apiUrl + '/transaksi_layanan/jenis?id_hewan='+this.formBayar.id_hewan
        ...
        if(this.uang<this.formBayar.total){
            ...
            this.$confirm("Yakin Menyelesaikan Pembayaran?").then(() => {
                ...
                this.bayar.append('total_harga', this.formBayar.total);
        ...
        rollback(data){
            ...
            this.$http.post(uri, this.return).then( (response) =>{
               this.snackbar = true;
                this.color = 'Green';
                this.text = "Transaksi "+data.kode+" telah dikembalikan ke Penjualan.";
                this.load = false;
                this.dialog = false;
                this.getDataBayar();
            }).catch(error =>{
        ...
        createPDF () {
            ...
            doc.setFontSize(10)
            doc.text(this.tglTransaksi, 380, 115, null, null, "right");
            doc.setFontSize(10)
            doc.text(20, 115, this.formBayar.kode)
            doc.setFontSize(10)
            doc.text(20, 130, "Member : "+this.formBayar.customer+"("+" "+this.formBayar.hewan+" "+"-"+" "+this.jenis+" "+")")
            if(this.formBayar.hewan=="Guest"){
                doc.setFontSize(10)
            ...
            doc.setFontSize(10)
            doc.setFontStyle("bold");
            doc.text(240, 195, "Harga")
            doc.setFontStyle("bold");
            doc.text(300, 195, "Jumlah")
            doc.setFontSize(10)
            doc.setFontStyle("bold");
            ...
            for (let i = 0, j = 1; i < this.detailLayanan.length; i++) {
                ...
                doc.text(315, 210+k , this.detailLayanan[i].jumlah)
