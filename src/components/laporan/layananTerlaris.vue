<template>
    <v-container>
        <v-card class="pa-md-4 mt-n12 mx-lg-auto" max-width="900">
             <h2 class="mb-4 text-center font-weight-black black--text">Laporan Layanan Terlaris</h2>
             <v-divider></v-divider>
...
export default {
    ...
    methods:{
        january(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=01&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.januari=response.data[0]
                console.log(this.januari)
            })
        },
        feb(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=02&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.februari=response.data[0]
                console.log(this.februari)
            })
        },
        march(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=03&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.maret=response.data[0]
                console.log(this.maret)
            })
        },
        apr(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=04&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.april=response.data[0]
                console.log(this.april)
            })
        },
        may(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=05&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.mei=response.data[0]
                console.log(this.mei)
            })
        },
        jun(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=06&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.juni=response.data[0]
            })
        },
        jul(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=07&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.july=response.data[0]
            })
        },
        agt(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=08&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.agustus=response.data[0]
            })
        },
        sept(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=09&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.september=response.data[0]
            })
        },
        okt(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=10&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.oktober=response.data[0]
            })
        },
        nov(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=11&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.november=response.data[0]
            })
        },
        des(){
            var uri = this.$apiUrl + '/laporan/layananTerlaris?bulan=12&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.desember=response.data[0]
            })
        },
        cetak(){
        ...
        createPDF () {
            let pdfName = "Laporan Layanan Terlaris"; 
            ...
            doc.text(130, 100, 'Laporan Layanan Terlaris')
            ...
            doc.text(146, 163, 'Nama Layanan')
            doc.text(330, 163, 'Jumlah')
            doc.setFontStyle("normal")
            //garis vertikal
            doc.setLineWidth(1);
            ...
            doc.setFontSize(11)
            doc.text(143, 186, this.januari.nama+" "+this.januari.jenis+" "+this.januari.ukuran);
            doc.text(143, 215, this.februari.nama+" "+this.februari.jenis+" "+this.februari.ukuran);
            doc.text(143, 242, this.maret.nama+" "+this.maret.jenis+" "+this.maret.ukuran);
            doc.text(143, 270, this.april.nama+" "+this.april.jenis+" "+this.april.ukuran);
            doc.text(143, 298, this.mei.nama+" "+this.mei.jenis+" "+this.mei.ukuran);
            doc.text(143, 328, this.juni.nama+" "+this.juni.jenis+" "+this.juni.ukuran);
            doc.text(143, 355, this.july.nama+" "+this.july.jenis+" "+this.july.ukuran);
            doc.text(143, 382, this.agustus.nama+" "+this.agustus.jenis+" "+this.agustus.ukuran);
            doc.text(143, 410, this.september.nama+" "+this.september.jenis+" "+this.september.ukuran);
            doc.text(143, 439, this.oktober.nama+" "+this.oktober.jenis+" "+this.oktober.ukuran);
            doc.text(143, 467, this.november.nama+" "+this.november.jenis+" "+this.november.ukuran);
            doc.text(143, 495, this.desember.nama+" "+this.desember.jenis+" "+this.desember.ukuran);

            doc.setFontSize(11)
            doc.text(340, 186, this.januari.jumlah);
            doc.text(340, 215, this.februari.jumlah);
            doc.text(340, 242, this.maret.jumlah);
            doc.text(340, 270, this.april.jumlah);
            doc.text(340, 298, this.mei.jumlah);
            doc.text(340, 328, this.juni.jumlah);
            doc.text(340, 355, this.july.jumlah);
            doc.text(340, 382, this.agustus.jumlah);
            doc.text(340, 410, this.september.jumlah);
            doc.text(340, 439, this.oktober.jumlah);
            doc.text(340, 467, this.november.jumlah);
            doc.text(340, 495, this.desember.jumlah);
            ...
            doc.setProperties({
                title: "Laporan Layanan Terlaris Tahun "+this.tahun
            });
