<template>
    <v-container>
        <v-card class="pa-md-4 mt-n12 mx-lg-auto" max-width="900">
             <h2 class="mb-4 text-center font-weight-black black--text">Laporan Produk Terlaris</h2>
             <v-divider></v-divider>
             <v-col class="d-flex" cols="12" sm="6">
                <v-select
                :items="items"
                label="Pilih Tahun"
                dense
                outlined
                class="mt-5"
                v-model="tahun"
                ></v-select>
            </v-col>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="#DEB887" fab large @click="cetak()">
                    <v-icon color="white">mdi-printer</v-icon>
                </v-btn>
            </v-card-actions>
        </v-card>
    </v-container>
</template>
<script>
import jsPDF from 'jspdf';
import 'jspdf-autotable'
export default {
    data() {
        return {
            tglTransaksi: new Date().toLocaleString(),
            items: ['2018', '2019', '2020'],
            tahun: '',
            produk: new Array(),
            januari: [],
            februari: [],
            maret: [],
            april: [],
            mei:[],
            juni: [],
            july: [],
            agustus: [],
            september: [],
            november: [],
            oktober: [],
            desember: [],
        }
    },
    methods:{
        january(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=01&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.januari=response.data[0]
                console.log(this.januari)
            })
        },
        feb(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=02&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.februari=response.data[0]
                console.log(this.februari)
            })
        },
        march(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=03&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.maret=response.data[0]
                console.log(this.maret)
            })
        },
        apr(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=04&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.april=response.data[0]
                console.log(this.april)
            })
        },
        may(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=05&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.mei=response.data[0]
                console.log(this.mei)
            })
        },
        jun(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=06&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.juni=response.data[0]
            })
        },
        jul(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=07&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.july=response.data[0]
            })
        },
        agt(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=08&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.agustus=response.data[0]
            })
        },
        sept(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=09&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.september=response.data[0]
            })
        },
        okt(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=10&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.oktober=response.data[0]
            })
        },
        nov(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=11&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.november=response.data[0]
            })
        },
        des(){
            var uri = this.$apiUrl + '/laporan/produkTerlaris?bulan=12&tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.desember=response.data[0]
            })
        },
        cetak(){
            console.log(this.tahun)
            this.january();
            this.feb();
            this.march();
            this.apr();
            this.may();
            this.jun();
            this.jul();
            this.agt();
            this.sept();
            this.nov();
            this.okt();
            this.des();
            this.konfirm();
        },
        konfirm(){
            this.$confirm("Cetak Laporan?").then(() => {
                this.createPDF();
            });
        },
        createPDF () {
            let pdfName = "Laporan Produk Terlaris"; 
            var doc = new jsPDF('2', 'pt', 'a5', true);

                        var img = 'data:image/jpeg;base64,'

            doc.addImage(img, "JPEG", 2, 1, 130, 80);
            
            doc.setFontSize(20)
            doc.text(180, 30, 'KOUVEE PET SHOP')
            doc.setFontSize(11)
            doc.text(150, 45, 'Jalan Moses GatotKaca No.22 Yogyakarta 55281')
            doc.setFontSize(11)
            doc.text(225, 60, 'Telp. (0274) 357735')
            doc.setFontSize(11)
            doc.text(195, 73, 'http://www.sayanghewan.com')
            // urutan line(padding, titik awal garis, panjang, titik akhir)
            doc.setLineWidth(0.5);
            doc.line(0, 80, 420, 80);
            doc.setFontSize(14)
            doc.setFontStyle("bold")
            doc.text(130, 100, 'Laporan Produk Terlaris')
            doc.setFontStyle("normal")
            
            doc.setFontSize(11)
            doc.text(30, 130, 'Tahun : ' + this.tahun)
            // Empty square
            doc.rect(30, 150, 360, 355);
            //header
            doc.setFontSize(11)
            doc.setFontStyle("bold")
            doc.text(37, 163, 'No')
            doc.text(80, 163, 'Bulan')
            doc.text(146, 163, 'Nama Produk')
            doc.text(330, 163, 'Jumlah')
            doc.setFontStyle("normal")
            //garis vertikal
            doc.setLineWidth(1);
            doc.line(60, 150, 60, 505);
            doc.setLineWidth(1);
            doc.line(133, 150, 133, 505);
            doc.setLineWidth(1);
            doc.line(313, 150, 313, 505);
            //garis horixontal
            doc.setLineWidth(0.5);
            doc.line(30, 170, 390, 170);
            //No
            doc.setFontSize(11)
            doc.text(40, 186, '1');
            doc.text(40, 215, '2');
            doc.text(40, 242, '3');
            doc.text(40, 270, '4');
            doc.text(40, 298, '5');
            doc.text(40, 328, '6');
            doc.text(40, 355, '7');
            doc.text(40, 382, '8');
            doc.text(40, 410, '9');
            doc.text(37, 439, '10');
            doc.text(37, 467, '11');
            doc.text(37, 495, '12');

            for (let i = 0, j = 20; i < 11; i++) {
                doc.setLineWidth(0.5);
                doc.line(30, 175+j, 390, 175+j);
                j+=28;
            }

            doc.setFontSize(11)
            doc.text(68, 186, 'January');
            doc.text(68, 215, 'February');
            doc.text(68, 242, 'Maret');
            doc.text(68, 270, 'April');
            doc.text(68, 298, 'Mei');
            doc.text(68, 328, 'Juni');
            doc.text(68, 355, 'July');
            doc.text(68, 382, 'Agustus');
            doc.text(68, 410, 'September');
            doc.text(68, 439, 'Oktober');
            doc.text(68, 467, 'November');
            doc.text(68, 495, 'Desember');

             doc.setFontSize(11)
            doc.text(143, 186, this.januari.nama);
            doc.text(143, 215, this.februari.nama);
            doc.text(143, 242, this.maret.nama);
            doc.text(143, 270, this.april.nama);
            doc.text(143, 298, this.mei.nama);
            doc.text(143, 328, this.juni.nama);
            doc.text(143, 355, this.july.nama);
            doc.text(143, 382, this.agustus.nama);
            doc.text(143, 410, this.september.nama);
            doc.text(143, 439, this.oktober.nama);
            doc.text(143, 467, this.november.nama);
            doc.text(143, 495, this.desember.nama);

             doc.setFontSize(11)
            doc.text(340, 186, this.januari.jumlah);
            doc.text(340, 215, this.februari.jumlah);
            doc.text(340, 242, this.maret.jumlah);
            doc.text(340, 270, this.april.jumlah);
            doc.text(340, 298, this.mei.jumlah);
            doc.text(340, 328, this.juni.jumlah);
            doc.text(340, 355, this.july.jumlah);
            doc.text(340, 382, this.agustus.jumlah);
            doc.text(340, 410, this.september.jumlah);
            doc.text(340, 439, this.oktober.jumlah);
            doc.text(340, 467, this.november.jumlah);
            doc.text(340, 495, this.desember.jumlah);

            doc.setFontStyle("normal")
            doc.setFontSize(10)
            doc.text("Tanggal Cetak : "+this.tglTransaksi, 391, 570, null, null, "right");
            

            doc.setProperties({
                title: "Laporan Produk Terlaris Tahun "+this.tahun
            });
            var string = doc.output('dataurlnewwindow');
            var embed = "<embed width='100%' height='100%' src='" + string + "'/>"
            var x = window.open();
            x.document.open();
            x.document.write(embed);
            x.document.close();

        },
    },
    mounted(){
        if (localStorage.getItem("token") != null) {
        if(localStorage.getItem("peran")=="Kasir"){
              window.location.replace('/homeKasir')
        }else if(localStorage.getItem("peran")=="Customer Service"){
              window.location.replace('/homeCS')
        }else if(localStorage.getItem("peran")=="Owner"){
              next()
        }
    }
    else{
      window.location.replace('/home')
    }
    },
}
</script>