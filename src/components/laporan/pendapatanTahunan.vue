...
export default {
    ...
    methods:{
        getPendapatanProduk(){
            var uri = this.$apiUrl + '/laporan/pendapatanProduk?tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.products=response.data[0]
                console.log(this.products)
            })
        },
        getPendapatanLayanan(){
            var uri = this.$apiUrl + '/laporan/pendapatanLayanan?tahun='+this.tahun
            this.$http.get(uri).then( (response) =>{
                this.layanan=response.data[0]
                console.log(this.layanan)
            })
        },
        cetak(){
            console.log(this.tahun)
            this.getPendapatanProduk();
            this.getPendapatanLayanan();
            this.konfirm();
        },
        konfirm(){
        ...
        createPDF () {
            ...
            doc.text(110, 100, 'Laporan Pendapatan Tahunan')
            ...
            doc.text(146, 163, 'Jasa Layanan')
            doc.text(253, 163, 'Produk')
            doc.text(335, 163, 'Total')
            doc.setFontStyle("normal")
            //garis vertikal
            doc.setLineWidth(1);
            ...
            doc.setProperties({
                title: "Laporan Pendapatan Tahun "+this.tahun
            });
